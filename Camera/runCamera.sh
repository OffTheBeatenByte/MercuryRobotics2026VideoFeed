...
# -crf 25              | Suggest using quality level 33.  Ranges from 0-52, with lower numbers giving better images, but worse compute time.  Here, we set it fairly high, as we are running on a fairly weak CPU, and evaluation time is at a premium
# -crf-max 30          | Maximum crf allowed (lowest quality of compressed image)
...
while [ 1 ]; do
  ffmpeg -re -s 640x480 -i $input -fflags +nobuffer -flags +low_delay -b:v 0 -c:v libx264 -preset ultrafast -tune zerolatency -rc-lookahead 0 -intra-refresh 1 -slice-max-size 1500 -g 1 -keyint_min 1 -crf 20 -crf_max 25 -f matroska - | nc $ip $port
